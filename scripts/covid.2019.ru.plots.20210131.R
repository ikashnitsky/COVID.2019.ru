# UPD  2021-01-31 ------------------------------
# Improve maps, uses simplified polygons which reder much faster
...
# UPD  2021-02-01 ------------------------------
# Ilya Kashnitsky: Cairo graphics are better in many ways
# consider changing to CairoPNG

Cairo::CairoPNG("../plots/01.COVID.2019.cumulated.TARD.png", height=750, width=1000, res=120, pointsize=10)
...
Cairo::CairoPNG("../plots/02.COVID.2019.cumulated.TARD.log10.png", height=750, width=1000, res=120, pointsize=10)
par(mar=c(6,5,4,5)+.1, lwd=2)
...
Cairo::CairoPNG("../plots/03.COVID.2019.CaseFatalityRatio.dyn.png", height=750, width=1000, res=120, pointsize=10)
par(mar=c(6,5,4,5)+.1)
...
# UPD  2021-02-01 ------------------------------
# Ilya Kashnitsky: maps with ggplot and sf

library(sf)
library(ggplot2)
library(dplyr)

# load the simplified shapefile object as an sf object
# see: simplify-and-reproject-spatial.R
ru_reg <- st_read("../misc/ESRI.shapefile/simplified.shp")
ru_bord <- st_read("../misc/ESRI.shapefile/borders.shp")

# join the spatial object with the stats dataframe
df_maps <- ru_reg %>% left_join(pop.derived, "LOCUS")
...
df_maps %>%
    ...
    theme(
        ...
        plot.background = element_rect(fill = "#001A33FF"),
        plot.title = element_text(hjust = 0.5, color = "#ffffff"),
        plot.caption = element_text(hjust = 0.5, color = "#ffffff")
    )+
    labs(
        title = "Russian Federation",
        caption = paste("Total COVID-2019 cases, as of", tail(RU.TIME, 1), "\nThe lighter, the less")
    )

p10 <- last_plot()

ggsave(
    "../plots/sf-10.COVID.2019.map.density.regions.png", p10,
    width = 7, height = 4.8, # these are inches with the default dpi 300
    type = "cairo-png"
)
...
df_maps %>%
    ggplot()+
    geom_sf(
        aes(fill = DETECTED.100K/max(DETECTED.100K)),
        color = NA
    )+
    geom_sf(data = ru_bord, size = .05, color = "#ffffff")+
    scale_fill_distiller(palette = 17, direction = 1)+
    cowplot::theme_map()+
    theme(
        legend.position = "none",
    ...
    labs(
        title = "Russian Federation",
        caption = paste("Total COVID-2019 cases per 100K, as of", tail(RU.TIME, 1), "\nThe lighter, the less")
    )

p11 <- last_plot()

ggsave(
    "../plots/sf-11.COVID.2019.map.density.regions.per_100K.png", p11,
    width = 7, height = 4.8,
    type = "cairo-png"
)
...
df_maps %>%
    ggplot()+
    geom_sf(
        aes(fill = DETECTED.7.100K/max(DETECTED.7.100K)),
        color = NA
    )+
    geom_sf(data = ru_bord, size = .05, color = "#ffffff")+
    scale_fill_distiller(palette = 17, direction = 1)+
    ...
    labs(
        title = "Russian Federation",
        caption = paste("COVID-2019 new cases over past 7 days per 100K, as of", tail(RU.TIME, 1), "\nThe lighter, the less")
    )

p12 <- last_plot()

ggsave(
    "../plots/sf-12.COVID.2019.map.density.regions.7_days_increment.per_100K.png",
    p12,
    width = 7, height = 4.8,
    type = "cairo-png"
)
...
df_maps %>%
    ggplot()+
    geom_sf(
        aes(fill = ACTIVE.100K/max(ACTIVE.100K)),
        color = NA
    )+
    geom_sf(data = ru_bord, size = .05, color = "#ffffff")+
    scale_fill_distiller(palette = 17, direction = 1)+
    ...
    labs(
        title = "Russian Federation",
        caption = paste("COVID-2019 active cases per 100K, as of", tail(RU.TIME, 1), "\nThe lighter, the less")
    )

p13 <- last_plot()

ggsave(
    "../plots/sf-13.COVID.2019.map.density.regions.active.per_100K.png",
    p13,
    width = 7, height = 4.8,
    type = "cairo-png"
)
...
df_maps %>%
    ggplot()+
    geom_sf(
        aes(fill = i.7.var.rmean.7/max(i.7.var.rmean.7)),
        color = NA
    )+
    geom_sf(data = ru_bord, size = .05, color = "#ffffff")+
    scale_fill_distiller(palette = 17, direction = 1)+
    ...
    labs(
        title = "Russian Federation",
        caption = paste("COVID-2019 mean excessive smoothing over past 7 days, as of", tail(RU.TIME, 1), "\nThe lighter, the less")
    )

p14 <- last_plot()

ggsave(
    "../plots/sf-14.COVID.2019.map.density.regions.i.7.var.rmean.7.png",
    p14,
    width = 7, height = 4.8,
    type = "cairo-png"
)
...
Cairo::CairoPNG("../plots/41.COVID.2019.hist.i.7.var.rmean.7.png", height=750, width=1000, res=120, pointsize=10)
...
Cairo::CairoPNG("../plots/42.COVID.2019.boxplot.i.7.var.rmean.7.png", height=750, width=500, res=120, pointsize=10)
...
Cairo::CairoPNG("../plots/15.COVID.2019.barplot.regions.png", height=1200, width=750, res=120, pointsize=10)
...
Cairo::CairoPNG("../plots/16.COVID.2019.barplot.regions.log.10.png", height=1200, width=750, res=120, pointsize=10)
par(mar=c(3.5,6,4,1)+.1, mgp=c(1.7,.5,-.5), cex.axis=.6)
...
Cairo::CairoPNG("../plots/17.COVID.2019.barplot.regions.per_100K.png", height=1200, width=750, res=120, pointsize=10)
par(mar=c(3.5,6,4,1)+.1, mgp=c(1.7,.5,-.5), cex.axis=.6)
...
Cairo::CairoPNG("../plots/18.COVID.2019.barplot.regions.per_100K.active.png", height=1200, width=750, res=120, pointsize=10)
par(mar=c(3.5,6,4,1)+.1, mgp=c(1.7,.5,-.5), cex.axis=.6)
...
Cairo::CairoPNG("../plots/19.COVID.2019.barplot.regions.per_100K.detected_for_the_past_7d.png", height=1200, width=750, res=120, pointsize=10)
